...
export class MusicBrainz {
  ...
  HdCoverArtUrl = async (mbid) => {
    let coverArtFetch = await (
      await fetch(`https://coverartarchive.org/release/${mbid}`)
    ).json();
    // Grab first image
    let res = coverArtFetch["images"][0]["image"]
  ...
  ReleaseInfo = async (mbid) => {
    let releaseFetch = await this.releaseFetch(
    ...
    // Populate artists
    for (let artist in releaseFetch["artist-credit"]) {
      res.artists.push({
    ...
    // Populate track list
    for (let track in releaseFetch["media"][0]["tracks"]) {
      res.trackList.push({
  ...
  ArtistInfo = async (mbid) => {
    let artistFetch = await this.releaseFetch(
    ...
    for (let releaseGroup in artistFetch["release-groups"]) {
      let resReleaseGroup = {
  ...
  SearchSongs = async (query) => {
    ...
    let data = await this.releaseFetch(
