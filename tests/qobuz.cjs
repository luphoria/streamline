...
const qobuz = async (query) => {
	...
	while (searchResults.length < totalResults) {
        console.log(`Searching for ${searchQuery} (${searchResults.length} results) . . . `);
        await new Promise(r => setTimeout(r, 1000));
		const searchReq = await fetch(`${QOBUZ_DL_API_URL}/api/get-music?q=${encodeURIComponent(searchQuery)}&offset=${searchResults.length}`,
			{
				headers: {
    ...
    resultsClean = resultsClean.filter(result => {return result.artist == queryArtist});
    resultsClean = resultsClean.filter(result => {return result.title == queryTitle});
    if (query[2]) { // release 
        resultsClean = resultsClean.filter(result => {return result.albumTitle == query[2]});
    }
    if (query[3]) { // duration (seconds)
        resultsClean = resultsClean.sort((a,b) => {
            return Math.abs(parseInt(query[3]) - a.duration) - Math.abs(parseInt(query[3]) - b.duration) 
        })
    }
    ...
    let data = await fetch(`${QOBUZ_DL_API_URL}/api/download-music?track_id=${resultsClean[0].id}&quality=27`, {
        "headers": {
          "accept": "application/json, text/plain, */*",
        },
        "method": "GET"
    ...
    await fs.writeFileSync(`tests/qobuz/${resultsClean[0].id}`,await Buffer.from(stream));
