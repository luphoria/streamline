...
export const get: Handler = async (req, res, next) => {
	...
	for (const track in releaseInfo.trackList) {
		...
		if (DBReq) {
			...
			stream = await t(sourceModules[source](artist, songTitle, releaseInfo.trackList[track].mbid, keywords));
			if (!stream.ok) {
				...
				for (let src in sourceModules) {
					usedSource = src;
					stream = await t(sourceModules[src](artist, songTitle, releaseInfo.trackList[track].mbid, keywords));
					if (stream.ok) {
						break;
					}
